...
- **Aerion's Storehouse is already chosen** (`ships/Night Fury.md`). This session is a natural moment to actually use it, logging the loot and the lockbox, per the inventory beat above.
- **The second facility is still open.** Surface the options at the table rather than deciding for them: **Library** (chart room, Orena's natural fit, runs Research, useful for the nav-data check below), **Sanctuary** (the quiet berth, no order attached, pure found-family value, could land well right after a session that opened on a fight and a death), or **Arcane Study** (the helm room, formalizing what Estra and Orena already use daily). See `rules/Rules & Systems.md` §6 for the full menu.
...
- **Repairs and payroll.** Settle the Reefside hull damage at 20 gp/HP and the month's operating cost before anything else gets fun.
