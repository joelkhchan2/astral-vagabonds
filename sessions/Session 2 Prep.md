...
2. **The system is fine.** First thing she settles, because it's the thing that will eat at Orena worst if left open: nobody died there. Whatever world was on the other end of that light is having an ordinary day right now, sun up, people going about it, none of them touched. **Leave how she knows unstated.** Not a personal trip, not a confirmed report, just the thing every captain out here seems to agree on without anyone quite saying where it comes from. What actually happens inside a guttered system stays a real, open mystery, for Orena and for the table.
...
- Establishes the core, table-facing rule of the Guttering: a guttered system is alive and intact, only unreachable, "cut off, not killed," stated as settled captain's-wisdom, not personally verified, so what actually happens inside stays genuinely unknown.
...
- **Storehouse (Aerion's first pick).** Roomy, Trade order, no prerequisite. Turns cargo space into steady income by buying low and selling high on bulk goods; this is the mechanical engine behind the side-gold the crew already earns most legs. Tonight's inventory beat (logging the loot, the lockbox, and the three new common items) is the room's first real use, not a new order yet, just populating it.
- **Library (Orena's first pick).** Roomy, Research order, no prerequisite. On a bastion turn, Research digs up information or lore, the direct mechanical version of "the survey work is secretly the investigation" that's already the campaign's design principle. Obvious hireling fit: Orena, chart room made literal. Research orders are a clean way to formally resolve later "what do we actually know about X" questions (a delisted route, a name from the lockbox, a rumor) instead of hand-waving it.
- **Arcane Study (Orena's second pick).** Roomy, Craft order, **requires an Arcane Focus or a tool used as a focus.** Nobody aboard is a Wizard/Sorcerer/Warlock with a true arcane focus, so this needed a table ruling to be legal at all: the ruling made was that the **spelljamming helm itself counts**, it's a magic item a spellcaster attunes to and channels power through daily, which is exactly what an arcane focus does. This formalizes the helm room Estra and Orena already use, and Craft output is an Arcane Focus, a Book, or a Spell Scroll.
- **Sanctuary (not picked).** Roomy, Craft order, requires a Holy Symbol or Druidic Focus. Orena's Paladin side would have qualified, but Library and Arcane Study were chosen instead. Still open for anyone who levels into more slots.
