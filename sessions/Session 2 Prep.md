...
- **The crew is 347th of 350, not 348th of 351.** Not from climbing, the *Grendel's Wake* is gone from the Board entirely, destroyed at Reefside. See `factions/The Manifest Board.md`.
- **Bex Halloway and the void scavver both stay offscreen.** Don't resolve either this session.
- **The lockbox is aboard, unopened, in Aerion's newly-formal Storehouse.** 5d8 glyph, confirmed.
- **Vellum isn't chasing anything.** They believe the box was lost at Reefside and are only watching. See `campaign/Story Spine.md`.
- **Sealed and Sworn is done and off the job board.** Three new postings are in rotation: Quiet Water, The Ledger Run, Two Chairs Short. See `rules/Contracts.md`.
- **The Guttering's mechanics are now settled** (what a dying current actually looks like, why dead reckoning doesn't work, what Orena watched on watch), see `campaign/Story Spine.md`, "The world truth." Worth rereading before running Estra's scene below, since she's explaining this from lived experience, not lecturing.
...
## 3. Aerion: inventory, Tack, and Flurry

Three linked beats, not three separate scenes, they can flow into each other over the course of the homecoming prep.

**Inventory and the ledger.** Aerion's natural spotlight: settling the loot from the cold open, reconciling the account against the 1,800 gp contract plus discretion tip and the 1,021 gp hoard (`ships/Night Fury.md`), and formally logging the lockbox into the new Storehouse facility, the first real use of the room with his name on it (`rules/Rules & Systems.md` §6). Let this be where the crew semi-officially decides who knows what about the box going forward.

**What's already in the Storehouse, if anyone asks (a good question to expect).** It's the cargo hold formalized into a real Trade facility (`rules/Bastions.md`), not empty shelving, so it should already read as a working supply room: dry rations and water in bulk (the 120-day air/food math this crew already lives by), spare hull plate and rope, the usual spelljammer odds and ends, plus whatever's left of the *Bottle Rocket*-adjacent junk and ordinary trade stock Aerion's been quietly turning over for side-gold. The lockbox and the fight's loot get logged into this same room tonight, which is what makes it feel real for the first time rather than a line on a character sheet.

**The stocking decision: three common magic items for the whole crew.** A good, concrete use of this scene: let Aerion's player pick **three common-rarity items** to add to the Storehouse as shared crew stock, paid for like any other supply purchase. Pull from `rules/Tables.md` §8 (Common Magic Items) for real options, and let him name something not on the list too, adjudicate it live and add it to the table afterward if it earns a place. This is Aerion doing what he actually does, provisioning the ship competently, and it's also just fun: hand three small magic toys to the whole crew and watch what everyone does with them over the next few sessions.

**A quiet one with Tack, and why the two of them specifically.** He saved the old ship's tattered flag and reattached it here; give Aerion a scene actually helping Tack with post-fight repairs, hull patching, a scorched line resecured, something with his hands. **The pairing isn't arbitrary.** Aerion and Tack are the two people on this crew who express devotion entirely through upkeep and competence, never through a word of it, Aerion with the ledger and the provisioning, Tack with his hands on the hull. Tack is also oblivious to social cues in a way Aerion never has to armor himself against, so he can say the earnest thing Aerion would never let himself say first: something like "she held. She always holds. I don't know what I'd do if she didn't hold," about the ship, unaimed at anyone, landing harder for it. Let Aerion just be present for it rather than having to say something back, that's the whole scene, one competent, undemonstrative person letting another one's undefended love for the same object land on him without flinching.
...
**House-ruled reminder:** this campaign pools the two PCs' level-5 facility slots into one shared total of **2** for the ship (`ships/Night Fury.md`), not two apiece, and either PC can issue any order once a room exists (`rules/Bastions.md`, "Combining bastions"). One slot's spent. Tonight is about the second, and it's real mechanical weight, not flavor, so worth walking through what each option actually does before the table picks.

- **Storehouse (already chosen, Aerion's).** Roomy, Trade order, no prerequisite. Turns cargo space into steady income by buying low and selling high on bulk goods; this is the mechanical engine behind the side-gold the crew already earns most legs. Tonight's inventory beat (logging the loot, the lockbox, and the three new common items) is the room's first real use, not a new order yet, just populating it.
- **Library, if picked.** Roomy, Research order, no prerequisite. On a bastion turn, Research digs up information or lore, the direct mechanical version of "the survey work is secretly the investigation" that's already the campaign's design principle. Obvious hireling fit: Orena, chart room made literal. This is the pick with the most immediate plot traction, since Research orders are a clean way to formally resolve later "what do we actually know about X" questions (a delisted route, a name from the lockbox, a rumor) instead of hand-waving it.
- **Sanctuary, if picked.** Roomy, Craft order, **requires a Holy Symbol or Druidic Focus.** Orena's Paladin side genuinely qualifies (a Paladin's spellcasting focus is a Holy Symbol), so the prerequisite is met, but there's no obvious hireling among the five named crew, none of them reads as a shrine-keeper. Craft output here is a Holy Symbol, a Druidic Focus, or minor blessed consumables, useful, but the least connected to anything currently live in the plot. Good pick if the table wants pure found-family/recovery value after a heavy session 1 and doesn't need it to do anything yet.
- **Arcane Study, if picked.** Roomy, Craft order, **requires an Arcane Focus or a tool used as a focus.** Nobody aboard is a Wizard/Sorcerer/Warlock with a true arcane focus, so this one needs a table ruling to be legal at all: the cleanest read is that the **spelljamming helm itself counts**, it's a magic item a spellcaster attunes to and channels power through daily, which is exactly what an arcane focus does. If the table's fine with that ruling, this formalizes the helm room Estra and Orena already use, and Craft output is an Arcane Focus, a Book, or a Spell Scroll. Flag the ruling explicitly if this is the pick, so it's a decision made once, out loud, not an inconsistency that surfaces later.

**Present all three, let the table choose.** No steer needed beyond the Arcane Study ruling, which has to be made regardless of whether they pick it, since the option existing at all depends on it.
...
- **Reefside is dockside knowledge by now, in rough, secondhand form.** The actual facts (a hired independent, a fatal ambush, a scavver, a wrecked checkpoint) travel faster and looser than the truth. Nobody outside the crew and Vellum knows the *Night Fury* was the "independent." Good texture: a regular at the Compass Rose retelling a wildly wrong third-hand version, or Sprat asking breathless questions that get half the details backward. Confirm or deny nothing; let the crew notice their own story getting away from them.
- **The Manifest Board update lands as a public moment, not just a paper fact.** 348th to 347th, posted at the Charter Window, and it's worth actually watching Estra read it: no climb, a neighbour's ship gone, a rank handed to her rather than earned. Finny's party instinct and Estra's flat non-reaction (`factions/The Manifest Board.md`) are the whole beat, no dialogue needed to land it.
- **Bex Halloway's neighbours have opinions, and they're wrong in different directions.** The *Cutlass Sunday*'s Vex reads her destruction as one less obstacle and says so with zero tact; the *Quiet Sum*'s Ollo Trask is genuinely rattled, a friendly rival actually dying does something to the bottom of the Board's small social world. Neither of them knows anything real, this is rumor and reaction, not a resolution to her open fate.
- **Repairs raise a real, small insurance decision.** The Reefside hull damage at 20 gp/HP can be paid straight out of the account, or filed against the crew's 1,800 gp Nightbreeze policy for partial reimbursement. Filing is the financially smart move and also puts a fatal ambush and a void scavver sighting on Nightbreeze's books against this specific hull, which is exactly the kind of claim history that nudges a premium up at the next renewal. A clean Aerion-shaped dilemma: save money now, or keep the file thin. Let him actually decide.
- **A banked, not-fired thread: House Darkshade's incident report.** `factions/The Manifest Board.md`'s "Running the Board" already flags Darkshade as the likely source of an anonymous incident report against a low-ranked vessel that just drew a founding house's attention. Reefside is exactly the kind of mess that report could hang on, unfair since the crew were the ones ambushed, but paperwork doesn't care. **Don't fire this tonight,** it's a good session 3-or-later surprise, a rank penalty that lands with no warning and forces the "the number measures paperwork, not truth" theme into a real scene. Just know it's sitting there, loaded.
- **Does finishing tonight's contract move the rank right now? No, and say so if it comes up.** Per the new Board mechanic (`factions/The Manifest Board.md`), a completed job banks a point toward the next quarterly posting rather than changing the number on the spot. That lag is worth naming out loud once, so the table understands why a clean win doesn't move a number tonight.

- **Repairs and payroll.** Settle the Reefside hull damage at 20 gp/HP (see the insurance beat above) and the month's operating cost before anything else gets fun.
...
- **Near-term Vellum tension, resolved to "quiet" for now.** Vellum isn't pursuing anything, they think the box was destroyed. Fine to let that land explicitly this session so the table can relax into it, the real trigger (a visit) doesn't fire until after the crew's *next* completed contract, not this one.
- **Board, places, and stores, checked and updated.** The Manifest Board's new 347th position (`factions/The Manifest Board.md`), whatever's changed at the Charter Window, the Float Market's one interesting thing, and the fresh rumour crop at the Compass Rose (`rules/Tables.md` §4).
...
Session 1 ran hotter than planned; this session counterweights toward warmth per the standing note in `CLAUDE.md`. **Session 3 can go further and actually relax**, this was the "heavy premiere," recovery session, and processing session all at once. A strong candidate for session 3: whichever contract gets picked tonight, played light, plus the Dog Show hook (`sessions/Episode - The International Space Dog Show.md`) if the timing and the table's mood are right.
